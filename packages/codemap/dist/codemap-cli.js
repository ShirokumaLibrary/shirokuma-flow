...
/** ADR-0012 + ADR-0024: error envelope with `error` / `message` / `help_hint`. */
function writeError(error, message, help_hint, pretty) {
    writeJson({ error, message, help_hint }, pretty);
}
function fail(error, message, help_hint, pretty) {
    writeError(error, message, help_hint, pretty);
    process.exit(1);
}
...
export function registerCodemapCommands(parent) {
    ...
    parent
        ...
        if (cmdOpts.planOnly && cmdOpts.execute) {
            fail('plan.conflicting_modes', 'error: --plan-only and --execute are mutually exclusive', 'pass --plan-only alone for a plan, or --execute --plan-hash <sha> --tx-id <ulid> to commit', pretty);
        }
        ...
        if (cmdOpts.execute) {
            if (!cmdOpts.planHash) {
                fail('plan.missing_hash', 'error: --execute requires --plan-hash <sha>', 'run with --plan-only first to obtain a plan_hash, then --execute --plan-hash <sha> --tx-id <ulid>', pretty);
            }
            if (!cmdOpts.txId) {
                fail('plan.missing_tx_id', 'error: --execute requires --tx-id <ulid>', 'copy the tx_id from the --plan-only envelope and pass it via --tx-id to preserve tx identity (ADR-0025 §3)', pretty);
            }
            const actualHash = computePlanHash(planHashInput);
            if (actualHash !== cmdOpts.planHash) {
                fail('plan.hash_mismatch', `error: --plan-hash ${cmdOpts.planHash} does not match current plan ${actualHash}`, 're-run --plan-only to obtain a fresh plan_hash (inputs may have changed)', pretty);
            }
...
//# sourceMappingURL=codemap-cli.js.map