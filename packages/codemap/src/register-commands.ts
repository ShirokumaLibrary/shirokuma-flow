...
function resolveProject(opts: GlobalOpts): string {
 ...
 */
export function registerCodemapCommands(parent: Command): void {
  parent
  ...
  parent
    .command('build')
    .description(
      'Build the codemap (main index + JIT bodies). ADR-0025 2-stage (no destructive ops).',
    )
    ...
    .action((cmdOpts: BuildCmdOpts) => {
        ...
        emitJson(
          {
            ...
            help_hint:
              'pass --plan-only alone for a plan, or --execute --plan-hash <sha> --tx-id <ulid> to commit',
          },
          { exitCode: 1, pretty },
        );
      }
      ...
      const planHashInput = { intents, diffs, destructive_ops: [] as const };
          ...
          emitJson(
            {
              ...
              help_hint:
                'run with --plan-only first to obtain a plan_hash, then --execute --plan-hash <sha> --tx-id <ulid>',
            },
            { exitCode: 1, pretty },
          );
        }
          ...
          emitJson(
            {
              ...
              help_hint:
                'copy the tx_id from the --plan-only envelope and pass it via --tx-id to preserve tx identity (ADR-0025 §3)',
            },
            { exitCode: 1, pretty },
          );
        }
          ...
          emitJson(
            {
              ...
              help_hint:
                're-run --plan-only to obtain a fresh plan_hash (inputs may have changed)',
            },
            { exitCode: 1, pretty },
          );
        }
      ...
      const envelope: PlanObservation = buildPlanEnvelope(planHashInput);
      writeJson(envelope, pretty);
    });

  parent.action(() => {
    writeJson({ package: PACKAGE_NAME }, parent.opts<GlobalOpts>().pretty ?? false);
  });
}
