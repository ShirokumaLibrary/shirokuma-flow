...
import { BatchExtractionResult, ExtractionResult } from '../../parsers/types/config.js';
...
export async function batchExtractCommand(options: BatchExtractOptions): Promise<number> {
  const spinner = ora('Loading configuration...').start();
...
async function generateReport(result: BatchExtractionResult, options: BatchExtractOptions): Promise<void> {
...
${
  result.results
    .filter((r: ExtractionResult) => !r.success)
    .map(
      (r: ExtractionResult) =>
        `### ${r.inputPath}
**Errors**:
