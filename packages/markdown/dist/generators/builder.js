...
export class Builder {
    ...
    async applyBuildOptimizations(content) {
        ...
        if (this.config.build.strip_heading_numbers) {
            const { remarkStripHeadingNumbers } = await import('../parsers/plugins/strip-heading-numbers.js');
        ...
        if (this.config.build.strip_section_meta) {
            const { remarkStripSectionMeta } = await import('../parsers/plugins/strip-section-meta.js');
            plugins.push(remarkStripSectionMeta);
        }
        // RAG Enhancement: Hierarchical heading normalization
        if (opts?.normalize_headings) {
            const { remarkNormalizeHeadings } = await import('../parsers/plugins/normalize-headings.js');
        ...
        if (opts?.remove_comments) {
            const { remarkRemoveComments } = await import('../parsers/plugins/remove-comments.js');
            plugins.push(remarkRemoveComments);
        }
        // Token Optimization: Remove badges
        if (opts?.remove_badges) {
            const { remarkRemoveBadges } = await import('../parsers/plugins/remove-badges.js');
            plugins.push(remarkRemoveBadges);
        }
        // Token Optimization: Remove duplicates (use with caution)
        if (opts?.remove_duplicates) {
            const { remarkRemoveDuplicates } = await import('../parsers/plugins/remove-duplicates.js');
        ...
        if (opts?.remove_internal_links) {
            const { remarkRemoveInternalLinks } = await import('../parsers/plugins/remove-internal-links.js');
            plugins.push(remarkRemoveInternalLinks);
        }
        // Token Optimization: Normalize whitespace
        if (opts?.normalize_whitespace) {
            const { remarkNormalizeWhitespace } = await import('../parsers/plugins/normalize-whitespace.js');
        ...
        if (opts?.normalize_whitespace) {
            const { normalizeWhitespaceContent } = await import('../parsers/plugins/normalize-whitespace.js');
