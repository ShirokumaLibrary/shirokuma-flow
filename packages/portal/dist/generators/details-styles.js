 ...
 */
export function generateTestCaseAnchorId(file, framework) {
 ...
 * テストファイルのカテゴリを判定
 */
export function getTestFileCategory(file, framework) {
 ...
 * カテゴリからスラッグを生成
 */
export function testCategoryToSlug(category) {
    return category.toLowerCase().replace(/\s+/g, "-");
}
/**
 * テストファイル名からスラッグを生成
 */
export function testFileToSlug(file) {
 ...
 */
export function generateTestPageUrl(file, framework, depth) {
...
// ===== JavaScript =====
export function getDetailScripts() {
    return `
...
// ===== 詳細ページスタイル =====
export function getDetailStyles(accentColor) {
    const colorVars = {
...
// ===== モジュールページ固有スタイル =====
export function getModuleSpecificStyles(accentColor) {
    const colorVars = {
        blue: "var(--accent-blue)",
        green: "var(--accent-green)",
        orange: "var(--accent-orange)",
