 ...
 * テストカテゴリのバッジを生成
 */
export function getCategoryBadgeHtml(category: TestCategory, count: number): string {
  if (count === 0) return "";

  const badges: Record<TestCategory, { icon: string; color: string; label: string }> = {
 ...
 */
export function getGlobalNavElements(depth: number): { headElements: string; bodyEndScripts: string } {
 ...
 * カテゴリ一覧ページのスタイル
 */
export function getCategoryListStyles(): string {
  return `
    .container {
      max-width: 1200px;
      margin: 0 auto;
 ...
 * ファイル一覧ページのスタイル
 */
export function getFileListStyles(): string {
  return `
    .container {
      max-width: 900px;
      margin: 0 auto;
 ...
 * テスト詳細ページのスタイル
 */
export function getTestDetailStyles(): string {
  return `
    .container {
      max-width: 900px;
      margin: 0 auto;
