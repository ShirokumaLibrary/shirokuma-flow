...
export function generateSearchIndex(
    ...
    for (const [moduleName, group] of Object.entries(featureMap.features)) {
      ...
      for (const table of group.tables || []) {
        items.push({
          ...
          title: table.name,
          description:
            table.description || `${moduleName}モジュールのDBテーブル`,
          category: "table",
          module: moduleName,
    ...
    if (uncategorized) {
      for (const screen of uncategorized.screens || []) {
        items.push({
          ...
          title: screen.name,
          description:
            screen.description || screen.descriptionEn || "未分類の画面",
          category: "screen",
    ...
    for (const table of dbSchema.tables) {
      ...
      if (existingIndex === -1) {
        items.push({
          ...
          title: table.name,
          description:
            table.description || `${table.category || "データベース"}テーブル`,
          category: "db",
  ...
  if (testCases) {
    const describeGroups = new Map<
      string,
      {
        tests: typeof testCases.testCases;
        firstTest: (typeof testCases.testCases)[0];
      }
    >();
