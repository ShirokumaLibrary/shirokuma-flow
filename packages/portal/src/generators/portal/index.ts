...
import {
  generateFeatureMapPage,
  generateFeatureMapAppPage,
} from "./pages/feature-map.js";
import {
  generateTestCasesPage,
  generateTestCasesFilePage,
  generateTestCaseDetailPage,
} from "./pages/test-cases.js";
import {
  generateDbSchemaPage,
  generateDbSchemaDbPage,
  generateDbSchemaTablePage,
  generateDbDiagramPage,
} from "./pages/db-schema.js";
import { generateI18nPage, generateI18nNamespacePage } from "./pages/i18n.js";
import {
  generatePackagesPage,
  generatePackageDetailPage,
} from "./pages/packages.js";
import { generateApiToolsPage } from "./pages/api-tools.js";
import {
  generateDetailsModulePage,
  generateDetailsItemPage,
  getItemsByType,
  type DetailType,
...
export class PortalGenerator {
  private options: PortalGeneratorOptions;
  private data: PortalData | null = null;
  ...
  async generate(): Promise<void> {
    ...
    if (!templatesExist()) {
      throw new Error(
        `ポータルテンプレートが見つかりません: ${getTemplatesDirPath()}`
      );
    }
   ...
   * 全ページを生成する
   */
  private buildAllPages(): PageEntry[] {
    if (!this.data) {
    ...
    if (data.available.hasDbSchema && data.dbSchema) {
      ...
      pages.push({
        ...
        html: wrap(
          "ER 図",
          generateDbDiagramPage(data),
          "/db-schema/diagram",
          [{ label: "DB スキーマ", href: "/db-schema" }, { label: "ER 図" }]
        ),
      });
      ...
      if (data.dbSchema.databases && data.dbSchema.databases.length > 1) {
        for (const db of data.dbSchema.databases) {
          ...
          pages.push({
            ...
            html: wrap(
              `DB: ${db.name}`,
              generateDbSchemaDbPage(data, db.name),
              `/db-schema/${dbSlug}`,
              [
          ...
          pages.push({
            ...
            html: wrap(
              `ER 図: ${db.name}`,
              generateDbDiagramPage(data, db.name),
              `/db-schema/${dbSlug}/diagram`,
              [
                { label: "DB スキーマ", href: "/db-schema" },
      ...
      for (const table of data.dbSchema.tables) {
        ...
        pages.push({
          ...
          html: wrap(
            `テーブル: ${table.name}`,
            generateDbSchemaTablePage(data, table.name),
            `/db-schema/${tableSlug}`,
            [
              { label: "DB スキーマ", href: "/db-schema" },
              { label: table.name },
    ...
    if (data.available.hasTestCases && data.testCases) {
      ...
      for (const file of files) {
        ...
        pages.push({
          ...
          html: wrap(
            `テスト: ${file.split("/").pop()}`,
            generateTestCasesFilePage(data, fileSlug),
            `/test-cases/${fileSlug}`,
            [
        ...
        // テスト詳細（行番号別）
        const testsInFile = data.testCases.testCases.filter(
          (tc) => tc.file === file
        );
        for (const test of testsInFile) {
          pages.push({
            ...
            html: wrap(
              test.it,
              generateTestCaseDetailPage(data, fileSlug, test.line),
              `/test-cases/${fileSlug}/${test.line}`,
              [
                { label: "テストケース", href: "/test-cases" },
                {
                  label: file.split("/").pop() || file,
    ...
    if (data.available.hasI18n && data.i18n) {
      ...
      for (const ns of data.i18n.namespaces) {
        ...
        pages.push({
          ...
          html: wrap(
            `i18n: ${ns.name}`,
            generateI18nNamespacePage(data, ns.name),
            `/i18n/${nsSlug}`,
            [{ label: "i18n", href: "/i18n" }, { label: ns.name }]
          ),
        });
      }
    }
    ...
    if (data.available.hasPackages && data.packages) {
      ...
      for (const pkg of data.packages.packages) {
        ...
        pages.push({
          ...
          html: wrap(
            `パッケージ: ${pkg.name}`,
            generatePackageDetailPage(data, pkg.name),
            `/packages/${pkgSlug}`,
            [
    ...
    if (data.available.hasFeatureMap && data.featureMap) {
      const types: DetailType[] = ["screen", "component", "action", "module", "table"];
      const allModules = [
      ...
      for (const type of types) {
        for (const moduleName of allModules) {
          ...
          pages.push({
            ...
            html: wrap(
              `${moduleName} - ${type}`,
              generateDetailsModulePage(data, type, moduleName),
              `/details/${type}/${moduleSlug}`,
              [
                { label: "詳細" },
                { label: type },
                { label: moduleName },
          ...
          if (group) {
            ...
            for (const item of items) {
              ...
              pages.push({
                ...
                html: wrap(
                  item.name,
                  generateDetailsItemPage(data, type, moduleName, item.name),
                  `/details/${type}/${moduleSlug}/${itemSlug}`,
                  [
                    { label: "詳細" },
                    {
    ...
    if (data.available.hasApplications && data.applications) {
      for (const app of data.applications.apps) {
        ...
        pages.push({
          ...
          html: wrap(
            app.name,
            generateAppsHomePage(data, app.id),
            `/apps/${appSlug}`,
            [{ label: "アプリ" }, { label: app.name }]
          ),
        });
        ...
        if (data.available.hasFeatureMap) {
          pages.push({
            ...
            html: wrap(
              `${app.name} - 機能マップ`,
              generateFeatureMapAppPage(data, app.id),
              `/apps/${appSlug}/feature-map`,
              [
                { label: "アプリ" },
   ...
   * 検索インデックスを生成して保存する
   */
  private generateSearchIndex(outputDir: string): void {
    if (!this.data) return;
    const existingPath = join(outputDir, "search-index.json");
    if (existsSync(existingPath)) return; // 既存インデックスを使用

    const index = generateSearchIndex(
      this.data.featureMap,
      this.data.dbSchema,
      this.data.testCases,
      this.data.details
    );
   ...
   */
  private writePages(pages: PageEntry[], outputDir: string): void {
