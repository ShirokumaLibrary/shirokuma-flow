 ...
 */
export function generateI18nPage(data: PortalData): string {
  if (!data.i18n) {
 ...
 */
export function generateI18nNamespacePage(
  data: PortalData,
  ...
  const namespace = data.i18n.namespaces.find(
    (ns) => ns.name === decoded || ns.name === namespaceName
  );
