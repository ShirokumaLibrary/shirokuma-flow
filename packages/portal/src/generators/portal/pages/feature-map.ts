 ...
 */
export function generateFeatureMapPage(data: PortalData): string {
  if (!data.featureMap) {
  ...
  const modules = Object.entries(data.featureMap.features).map(
    ([name, group]) => ({
      name,
  ...
  const hasUncategorized =
    (uncategorized.screens?.length || 0) +
  ...
  const totalItems = modules.reduce(
    (sum, m) =>
      sum + m.screenCount + m.componentCount + m.actionCount + m.tableCount,
    0
  );
 ...
 */
export function generateFeatureMapAppPage(
  data: PortalData,
  appId: string
): string {
  if (!data.featureMap) {
  ...
  const modules = Object.entries(data.featureMap.features)
    .map(([name, group]) => ({
      ...
      screens: (group.screens || []).filter(
        (s) =>
          !s.app || s.app.toLowerCase() === appId.toLowerCase() || s.app === "Shared"
      ),
      components: (group.components || []).filter(
        (c) =>
          !c.app || c.app.toLowerCase() === appId.toLowerCase() || c.app === "Shared"
      ),
      actions: (group.actions || []).filter(
        (a) =>
          !a.app || a.app.toLowerCase() === appId.toLowerCase() || a.app === "Shared"
      ),
      tables: group.tables || [],
    }))
    .filter(
      (m) =>
        m.screens.length + m.components.length + m.actions.length > 0
    )
    .map((m) => ({
      ...m,
