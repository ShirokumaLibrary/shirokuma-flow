...
import type { PortalData, DbTable } from "../types.js";
 ...
 */
export function generateDbSchemaPage(data: PortalData): string {
  if (!data.dbSchema) {
 ...
 * 特定 DB のスキーマページを生成する
 */
export function generateDbSchemaDbPage(
  data: PortalData,
  dbName: string
): string {
  if (!data.dbSchema) {
 ...
 * テーブル詳細ページを生成する
 */
export function generateDbSchemaTablePage(
  data: PortalData,
  tableName: string
): string {
  if (!data.dbSchema) {
  ...
  const table = data.dbSchema.tables.find(
    (t) => t.name === decodedName || t.name === tableName
  );
 ...
 * DB ER 図ページを生成する
 */
export function generateDbDiagramPage(
  data: PortalData,
  dbName?: string
): string {
  if (!data.dbSchema) {
...
function groupTablesByCategory(tables: DbTable[]): CategoryGroup[] {
...
function generateMermaidEr(tables: DbTable[]): string {
