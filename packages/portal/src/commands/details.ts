 ...
 */
export function detailsCommand(options: DetailsOptions): number {
  ...
  const featureMap = JSON.parse(readFileSync(featureMapPath, "utf-8")) as FeatureMap;
  logger.info(t("commands.details.featureMapLoaded"));

  // test-cases.json 読み込み（オプション）
  if (existsSync(testCasesPath)) {
    const testCasesJson = JSON.parse(readFileSync(testCasesPath, "utf-8")) as TestCasesJson;
