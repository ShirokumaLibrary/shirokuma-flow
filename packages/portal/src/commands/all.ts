 ...
 */
export async function cmdGenerateAll(options: GenerateAllOptions): Promise<number> {
  ...
  const steps: Array<{ name: string; fn: (opts: GenerateAllOptions) => unknown }> = [
  ...
  // --with-github は portal では未サポート（github-data は flow に残留）
