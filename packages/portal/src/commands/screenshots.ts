 ...
 * screenshots コマンド
 */
export async function screenshotsCommand(options: ScreenshotsOptions): Promise<number> {
 ...
 * アノテーションから画面を収集
 */
function collectFromAnnotations(
  projectPath: string,
  config: Required<ScreenshotsConfig>,
  logger: Logger
): ScreenItem[] {
 ...
 */
function collectFromFeatureMap(
  projectPath: string,
  ...
  try {
    featureMap = JSON.parse(featureMapContent) as FeatureMap;
  } catch {
...
function convertToDynamicRoute(
  ...
  const convertedRoute = route.replace(paramPattern, (_match, paramName: string) => {
    isDynamic = true;
