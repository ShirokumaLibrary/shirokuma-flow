 ...
 * schema コマンドハンドラ
 */
export async function schemaCommand(options: SchemaOptions): Promise<number> {
  ...
  for (let i = 0; i < schemaConfigs.length; i++) {
    ...
    const dbTables = generateSchemaIndex(
      schemaConfig.source,
      schemaConfig.output,
      dbmlPath,
      logger,
...
function generateSchemaIndex(
  ...
  logger: Logger,
  _dbName: string = "default"
): PortalTable[] | undefined {
