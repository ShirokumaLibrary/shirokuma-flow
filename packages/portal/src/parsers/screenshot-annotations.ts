 ...
 * ```
 */
export function parseScreenshotAnnotations(
  content: string,
  filePath: string
 ...
 * ```
 */
export function scanFilesForScreenshots(
 ...
 */
function extractTags(jsdocBlock: string): {
 ...
 */
function extractDescription(jsdocBlock: string): string | undefined {
  const lines = jsdocBlock.split("\n");
