 ...
 * ```
 */
export function inferActionTypeFromPath(filePath: string): ActionType | undefined {
  const normalizedPath = filePath.replace(/\\/g, "/").toLowerCase();

  // Check for crud/ directory
  if (
    normalizedPath.includes("/lib/actions/crud/") ||
  ...
  if (
    normalizedPath.includes("/lib/actions/domain/") ||
