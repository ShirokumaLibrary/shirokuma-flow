...
// 素の `npx jest` では動作しない（Node.js の構造的制約）。
...
const config = {
  ...
  injectGlobals: true,
  // Limit parallelism to prevent resource contention in CLI integration tests
  // (init, update-skills, discussion-templates run the CLI binary and can
  //  interfere when too many workers are spawned simultaneously)
  maxWorkers: 4,
};
