...
スキル内で異常系（誤ブランチ、Issue が想定外ステータス、未コミット変更、リソースの欠落等）に遭遇した場合の処理パターン:

| シナリオ | 推奨パターン | 避けるパターン |
|---------|------------|--------------|
| 破壊的操作の確認（リセット・キャンセル・上書き） | `AskUserQuestion` で意図確認 → 確認後に CLI を自動実行 | 確認なしで自動実行 |
| 単純な遷移エラー（既に Done、既にアサイン済み等） | 警告ログ + 続行（冪等処理） | エラーで停止 |
| 復旧手順が複数ステップ（PR revert、ブランチ復旧等） | エラーメッセージ + **推奨コマンドをユーザーに提示** + 制御を返す | 自動的に git/gh を連発 |
| ヘッドレスモード | デフォルト動作を明記 + UCP として停止条件を定義（`implement-flow` の W1〜W5 参照） | 通常モードと同じ AskUserQuestion を呼ぶ（インタラクティブ環境がない） |

「推奨コマンドをユーザーに提示し制御を返す」パターンは、特に `git push --force` / `branch -D` / `reset --hard` 等の **取り消し困難な操作** や、複数ステップのリカバリー手順で適用する。スキル内で自動実行するとユーザーが状況を理解する前に変更が発生し、ロールバックが難しくなる。
