...
| Status | 説明 | 遷移トリガー |
|--------|------|-------------|
| In progress | PR 作成直後（AI 自己レビュー中） | `pr create` が PR を Projects に追加時に自動設定 |
| Review | AI 自己レビュー完了後（コードレビュー待ち） | `submit N` で遷移 |
...
| PR | `pr create` 時（コードレビュー可能） | `pr merge` → Done |
...
| PR レビュー待ち | 計画 Issue | In progress のまま（PR 自身が Review） | `pr create` |
