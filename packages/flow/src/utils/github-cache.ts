 ...
 */
function typeToDir(type: "issue" | "pull_request" | "discussion"): string {
 ...
 * @returns ファイルパス
 */
export function getCommentCachePath(
  number: number,
  databaseId: number,
  type: "issue" | "pull_request" | "discussion",
  org: string,
 ...
 */
export function getOpenIssuesIndexPath(
  org: string,
  repo: string,
  baseDir: string = GITHUB_CACHE_DIR
): string {
 ...
 */
function serializeCache(metadata: Record<string, unknown>, body: string): string {
 ...
 */
function deserializeCache(raw: string): { metadata: Record<string, unknown>; body: string } | null {
  const parsed = parseFrontmatter(raw);
  if (!parsed.hasFrontmatter || parsed.parseError || !parsed.data) return null;
...
export function writeCache(
  number: number,
  metadata: Omit<CacheMetadata, "cached_at">,
  body: string = "",
  org: string,
  repo: string,
  baseDir: string = GITHUB_CACHE_DIR
...
export function readCache(
  ...
  if (typeof meta.number !== "number" || typeof meta.type !== "string") return null;

  return {
    metadata: meta as unknown as CacheMetadata,
    body: parsed.body,
    raw,
  };
}
 ...
 * @param baseDir - キャッシュのベースディレクトリ
 */
export function writeCommentCache(
  number: number,
  databaseId: number,
  metadata: Omit<CommentCacheMetadata, "cached_at">,
  body: string = "",
  type: "issue" | "pull_request" | "discussion",
  org: string,
  repo: string,
  ...
  const fullMetadata: CommentCacheMetadata = {
    ...metadata,
    cached_at: new Date().toISOString(),
  };
  const content = serializeCache({ ...fullMetadata }, body);
 ...
 */
export function readCommentCache(
  number: number,
  databaseId: number,
  type: "issue" | "pull_request" | "discussion",
  org: string,
  ...
  // 型チェック: 必須フィールドが揃っているか確認
  const meta = parsed.metadata;
  if (typeof meta.number !== "number" || typeof meta.database_id !== "number") return null;

  return {
    metadata: meta as unknown as CommentCacheMetadata,
    body: parsed.body,
    raw,
  };
}
...
export function probeReadCache(
  number: number,
  org: string,
  repo: string,
  baseDir: string = GITHUB_CACHE_DIR
): CacheEntry | null {
  for (const t of ["issue", "pull_request", "discussion"] as const) {
 ...
 * @param baseDir - キャッシュのベースディレクトリ
 */
export function writeOpenIssuesIndex(
  entries: OpenIssueEntry[],
  org: string,
  repo: string,
  baseDir: string = GITHUB_CACHE_DIR
): void {
 ...
 */
export function readOpenIssuesIndex(
  org: string,
  repo: string,
  baseDir: string = GITHUB_CACHE_DIR
): OpenIssueEntry[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const type: "issue" | "pull_request" = typeStr === "PR" ? "pull_request" : "issue";
    const title = cols[2] ?? "";
 ...
 */
export function upsertOpenIssuesEntry(
  entry: OpenIssueEntry,
  org: string,
  repo: string,
  baseDir: string = GITHUB_CACHE_DIR
): void {
 ...
 */
export function removeOpenIssuesEntry(
  number: number,
  org: string,
  repo: string,
  baseDir: string = GITHUB_CACHE_DIR
 ...
 * @param baseDir - キャッシュのベースディレクトリ
 */
export function rebuildOpenIssuesIndex(
  org: string,
  repo: string,
  baseDir: string = GITHUB_CACHE_DIR
): void {
  ...
  for (const dir of [issuesDir, pullsDir]) {
    const type: "issue" | "pull_request" = dir === issuesDir ? "issue" : "pull_request";
    let files: string[] = [];
    try {
    ...
    for (const filePath of files) {
      ...
      const meta = parsed.metadata;
      if (typeof meta.number !== "number") continue;
