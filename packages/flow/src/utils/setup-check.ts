 ...
 * - Discussion categories (ADR, Knowledge, Research) with recommended settings
 ...
 *
 * @since #2385 Handovers カテゴリを必須から除外（引き継ぎは Issue 一元化方針へ移行）
 */
...
const REQUIRED_DISCUSSION_CATEGORIES = ["ADR", "Knowledge", "Research"];
...
function checkProjectExists(projectId: string | null): SetupCheckItem[] {
  return [{
    ...
    hint: projectId
      ? undefined
      : "Create a GitHub Project: shirokuma-flow projects create-project --title \"<repo-name>\" --lang ja",
  }];
}
...
async function checkProjectFields(projectId: string): Promise<SetupCheckItem[]> {
  ...
  return REQUIRED_PROJECT_FIELDS.map((fieldName) => {
    ...
    return {
      ...
      hint: resolved
        ? undefined
        : `Create "${fieldName}" field: Run 'shirokuma-flow projects setup --lang ja' ` +
  ...
  return allFieldNames.map((fieldName) => ({
    ...
    hint: fieldName in fields
      ...
      : `Create Text field "${fieldName}": Project → Settings → Custom fields → New field (Text). ` +
        `Or run 'shirokuma-flow projects setup-metrics' for automated setup.`,
  }));
}
  ...
  for (const cat of categoryOrder) {
    ...
    for (const item of catItems) {
      if (item.ok) {
        ...
        if (item.hint) logger.debug(`    -> ${item.hint}`);
        if (item.url) logger.debug(`    URL: ${item.url}`);
        if (item.recommended) {
