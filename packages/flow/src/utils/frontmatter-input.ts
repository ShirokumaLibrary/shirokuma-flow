 ...
 */
const FIELD_TO_OPTION_MAP: Record<string, string> = {
  type: "issueType",
  // labels は配列のまま渡す
};
 ...
 */
const SAFE_FIELDS: Record<CommandType, Set<string>> = {
...
export function parseFrontmatterInput(
  ...
  if (parsed.hasFrontmatter && parsed.data) {
    for (const [key, value] of Object.entries(parsed.data)) {
      if (!safeFieldSet.has(key)) continue;
 ...
 */
export function mergeFrontmatterOptions(
  frontmatterData: FrontmatterInput,
