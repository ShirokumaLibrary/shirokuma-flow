...
export function createStatusCommand(): Command {
  const status = new Command("status")
    .description(
      "Issue ステータス管理 (transition, approve, update-batch, get, allowed, history)"
    );
  ...
  status
    ...
    .option("--via <status>", "中間ステータスを経由して 2 段階遷移する（例: --to Review --via 'In progress' で ToDo → In progress → Review）")
  ...
  status
    ...
    .action(async (localOpts, command: Command) => {
      ...
      if (!resolveFileOption(options, "issueCommentFile", "--issue-comment-file")) return;
