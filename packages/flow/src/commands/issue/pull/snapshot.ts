...
import {
  GRAPHQL_QUERY_ISSUE_WITH_FIELDS,
  IssueQueryResult,
  resolveIssueProjectFields,
} from "./issue.js";
import {
  GRAPHQL_QUERY_DISCUSSION_WITH_FIELDS,
...
export async function fetchRemoteSnapshot(
  ...
  if (type === "discussion") {
    // Discussion のみ取得
    const result = await runGraphQL<DiscussionQueryResult>(GRAPHQL_QUERY_DISCUSSION_WITH_FIELDS, graphqlVars);
  ...
  if (issueSnapshot) return issueSnapshot;

  const discResult = await runGraphQL<DiscussionQueryResult>(GRAPHQL_QUERY_DISCUSSION_WITH_FIELDS, graphqlVars);
 ...
 */
async function fetchRemoteIssueSnapshot(
  owner: string,
  repo: string,
  number: number,
  graphqlVars: { owner: string; name: string; number: number }
): Promise<RemoteItemSnapshot | null> {
  const issueResult = await runGraphQL<IssueQueryResult>(GRAPHQL_QUERY_ISSUE_WITH_FIELDS, graphqlVars);
