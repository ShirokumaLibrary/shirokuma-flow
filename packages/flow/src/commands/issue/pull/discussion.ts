...
interface DiscussionCommentReplyNode {
  id?: string;
  databaseId?: number;
  author?: { login?: string };
  body?: string;
...
interface DiscussionCommentNode {
  ...
  replies?: {
    totalCount?: number;
    nodes?: DiscussionCommentReplyNode[];
  };
}
 ...
 */
export async function fetchAndCacheDiscussion(
  owner: string,
  repo: string,
  number: number,
  org: string,
  ...
  const [discussionResult, commentsResult] = await Promise.all([
    runGraphQL<DiscussionQueryResult>(GRAPHQL_QUERY_DISCUSSION_WITH_FIELDS, graphqlVars),
    runGraphQL<DiscussionCommentsQueryResult>(GRAPHQL_QUERY_DISCUSSION_COMMENTS, graphqlVars),
  ]);
