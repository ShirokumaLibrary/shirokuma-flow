...
interface DiscussionInfoResult {
  data?: {
    repository?: {
      discussion?: {
        number?: number;
 ...
 */
function inferLinkType(category: string | undefined): string {
 ...
 */
function parseLinkSection(body: string): LinkEntry[] {
 ...
 */
function upsertLinkSection(body: string, entry: LinkEntry): string {
 ...
 */
function removeLinkFromSection(body: string, discussionNumber: number): string {
 ...
 * Issue と Discussion のリンクを管理する。
 */
export async function cmdItemLink(
  numberStr: string,
  options: LinkOptions,
  logger: Logger
): Promise<number> {
...
async function addLink(
  ...
  // Discussion 情報を取得
  const discResult = await runGraphQL<DiscussionInfoResult>(
    GRAPHQL_QUERY_DISCUSSION_INFO,
  ...
  const updateResult = await runGraphQL<UpdateIssueResult>(
    GRAPHQL_MUTATION_UPDATE_ISSUE_BODY,
    { id: issueId, body: newBody }
  );
...
async function listLinks(
  ...
  // 各 Discussion の詳細を取得
  const detailedLinks: LinkEntry[] = [];
  for (const link of links) {
    const discResult = await runGraphQL<DiscussionInfoResult>(
      GRAPHQL_QUERY_DISCUSSION_INFO,
      { owner, name: repo, number: link.discussion }
    );
