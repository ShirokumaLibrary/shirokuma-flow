 ...
 */
export async function cmdItemParent(
  ...
  try {
    ...
    if (message.includes("422") || message.includes("already has a parent") || message.includes("sub_issue_id")) {
      logger.error(
        `Issue #${issueNumber} は既に親 Issue に紐付けられています。` +
 ...
 */
async function autoSetChildFields(
  owner: string,
  repo: string,
  issueNumber: number,
  logger: Logger
  ...
  const result = await runGraphQL<ChildFieldsResult>(GRAPHQL_QUERY_CHILD_FIELDS, {
    owner,
    name: repo,
    number: issueNumber,
  });

  if (!result.success) return autoUpdates;

  const issue = result.data.data?.repository?.issue;
  if (!issue?.id) return autoUpdates;
 ...
 */
export async function cmdItemUnparent(
  issueNumberStr: string,
...
`;
    type ParentQueryResult = {
      data?: { repository?: { issue?: { parent?: { number?: number } } } };
    };
    const parentResult = await runGraphQL<ParentQueryResult>(QUERY_PARENT, {
      owner,
      name: repo,
      number: issueNumber,
    });
