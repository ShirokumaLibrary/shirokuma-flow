...
import { STATUS_VALUES, LEGACY_STATUS_VALUES, isBackwardTransition, validateTransition } from "../../../utils/status-workflow.js";
import { resolveAndUpdateStatus, updateProjectStatus } from "../../../utils/issue-detail.js";
import {
  GRAPHQL_MUTATION_CLOSE_ISSUE,
  GRAPHQL_MUTATION_REOPEN_ISSUE,
...
/** ローカルとリモートの配列差分を計算する */
export function computeArrayDiff(local: string[] | null, remote: string[]): { toAdd: string[]; toRemove: string[] } {
  if (local === null) return { toAdd: [], toRemove: [] };
 ...
 *
 * 重要遷移: → Review, → Done, 後退遷移（補償差し戻し）
 * 通常遷移（ToDo → In Progress 等）は GitHub Projects Activity ログに委ねる。
 * #2439: Completed は LEGACY 値だが既存 Issue のために継続して処理
 */
const IMPORTANT_TARGET_STATUSES: ReadonlySet<string> = new Set([
  LEGACY_STATUS_VALUES.COMPLETED_LEGACY, // #2440: "Completed" LEGACY → Done（既存 Issue の保険）
  STATUS_VALUES.REVIEW,
  STATUS_VALUES.DONE,
]);

/** 重要遷移の場合に自動コメントを投稿する（best-effort） */
async function postStatusTransitionComment(
  issueId: string,
  number: number,
  from: string,
  to: string,
  ...
  if (!IMPORTANT_TARGET_STATUSES.has(to) && !backward) return;
...
export async function pushIssueBody(
  ...
  let pendingStatusValue: string | undefined;
  const projectFieldChanges: { Priority?: string; Size?: string } = {};
  ...
  let resolvedIssueTypeId: string | null | undefined;
  ...
  if (hasNonStatusChanges || pendingStatusValue) {
    if (matchingItem?.id && matchingItem?.project?.id) {
      ...
      // Status ガードは非 Status フィールド書き込み前に評価する。
      // issue push には --rollback フラグがないためロールバック遷移は --force のみで許可。
      // 先に評価することで、ガード拒否時に Priority/Size だけ更新される部分更新を防ぐ。
      if (pendingStatusValue && remoteStatus) {
        const transitionCheck = validateTransition("issue", remoteStatus, pendingStatusValue, { allowRollback: false });
        if (!transitionCheck.valid) {
          if (!options.force) {
            logger.error(transitionCheck.error!);
            return 1;
          }
          logger.warn(`${transitionCheck.error!} (--force で続行)`);
        }
      }
  ...
  if (changes.state && issueId) {
    if (localState === "CLOSED") {
      ...
      interface CloseResult {
        data?: { closeIssue?: { issue?: { id?: string; number?: number; state?: string } } };
      }
      ...
      interface ReopenResult {
        data?: { reopenIssue?: { issue?: { id?: string; number?: number; state?: string } } };
      }
