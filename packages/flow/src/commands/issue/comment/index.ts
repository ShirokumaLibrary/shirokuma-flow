...
import {
  GRAPHQL_MUTATION_ADD_DISCUSSION_COMMENT,
  getDiscussionId,
...
import type { AddCommentOptions } from "../../items/types.js";
 ...
 *
 * @param numberStr - アイテム番号
 * @param file - コメント本文のファイルパス
...
export async function cmdAddComment(
  ...
  if (issueId) {
    return addIssueComment(owner, repo, number, issueId, body, file, "issue", logger);
  }

  // PR として試みる
  const prId = await getPullRequestId(owner, repo, number);
  if (prId) {
    return addIssueComment(owner, repo, number, prId, body, file, "pull_request", logger);
  }

  // Discussion として試みる
  const discussionId = await getDiscussionId(owner, repo, number);
  if (discussionId) {
    return addDiscussionComment(owner, repo, number, discussionId, body, file, logger);
  }
...
async function addIssueComment(
  ...
  writeCommentCache(
    number,
    comment.databaseId,
    { number, database_id: comment.databaseId },
    body,
    itemType,
    owner,
    repo
