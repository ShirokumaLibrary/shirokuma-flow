 ...
 *
 * 各種コメント骨格 (review-report|review-response|completion-report) を
 * 生成する純粋関数。
 *
 * @since #2385 handover タイプを廃止（引き継ぎは Issue 一元化方針で
 *   In progress の Issue 本体 + コメントを直接使う）
 */

import type { Locale } from "../../../utils/i18n.js";

/** コメントタイプ */
export type CommentType = "review-report" | "review-response" | "completion-report";

/** JA テンプレート */
const COMMENT_TEMPLATES_JA: Record<CommentType, string> = {
...
/** EN テンプレート */
const COMMENT_TEMPLATES_EN: Record<CommentType, string> = {
 ...
 * @returns テンプレート文字列
 */
export function generateCommentTemplate(lang: Locale, type: CommentType): string {
