...
import { getLocale, type Locale } from "../../../utils/i18n.js";
import type { PlanLevel } from "./plan.js";
 ...
 */
function writeOutput(result: string, outputPath?: string): void {
  if (outputPath) {
...
export function createTemplateCommand(): Command {
  ...
  template
    ...
    .action(async (localOpts) => {
      ...
      const result = generateIssueTemplate(localOpts.lang as Locale);
      writeOutput(result, localOpts.output as string | undefined);
    });
  ...
  template
    ...
    .action(async (localOpts) => {
      ...
      const result = generatePlanTemplate(localOpts.lang as Locale, localOpts.level as PlanLevel);
      writeOutput(result, localOpts.output as string | undefined);
    });
  ...
  template
    ...
    .action(async (localOpts) => {
      ...
      const result = generatePrTemplate(localOpts.lang as Locale);
      writeOutput(result, localOpts.output as string | undefined);
    });
  ...
  template
    ...
    .action(async (localOpts) => {
      ...
      const result = generateAdrTemplate(localOpts.lang as Locale);
      writeOutput(result, localOpts.output as string | undefined);
    });
  ...
  template
    ...
    .addOption(new Option("--type <type>", "コメントタイプ (review-report|review-response|completion-report)").choices(["review-report", "review-response", "completion-report"]).default("completion-report"))
    .option("--output <file>", "出力ファイルパス")
    .addOption(langOption())
    .action(async (localOpts) => {
      const { generateCommentTemplate } = await import("./comment.js");
      const result = generateCommentTemplate(localOpts.lang as Locale, localOpts.type as CommentType);
      writeOutput(result, localOpts.output as string | undefined);
    });
