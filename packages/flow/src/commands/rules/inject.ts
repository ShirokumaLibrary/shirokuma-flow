 ...
 * @returns ルールエントリ配列
 */
function loadRules(rulesDir: string, stripFrontmatter: boolean): RuleEntry[] {
  ...
  for (const filePath of filePaths) {
    try {
      ...
      entries.push({
        ...
        frontmatterData: parsed.data as Record<string, unknown> | undefined,
 ...
 * @returns マッチすれば true
 */
function matchesScope(scopeField: unknown, targetScope: string): boolean {
 ...
 * rules inject コマンド実装
 */
export async function rulesInjectCommand(options: RulesInjectOptions): Promise<void> {
  ...
  if (existsSync(shirokumaRulesDir)) {
    ...
    const langDir: "ja" | "en" = (langOpt === "ja" || langOpt === "en")
      ? langOpt
      : getLanguageSetting(project) === "japanese" ? "ja" : "en";
    const bundledPluginPath = langDir === "ja" ? getBundledPluginPathJa() : getBundledPluginPath();
    const bundledRulesDir = join(bundledPluginPath, "rules");
    if (!existsSync(bundledRulesDir)) {
      console.error(
        `Error: ルールディレクトリが見つかりません。\n` +
        `  確認したパス: ${bundledRulesDir}\n` +
        `  shirokuma-flow init を実行してプラグインをインストールしてください。`
      );
      setExitCode(1);
      return;
    }
    entries = loadRules(bundledRulesDir, false);
  }
