...
import {
  GRAPHQL_QUERY_PR_REVIEW_THREADS,
  transformReviews,
  transformThreads,
  transformIssueComments,
  type ReviewNode,
  type ThreadNode,
...
// =============================================================================
// Output Type (#1752 — 型安全性改善)
// =============================================================================

interface PrShowOutput {
  [key: string]: unknown;
  number: number;
  title: string;
  state: string;
  ...
  // withComments fields (show コマンド経由のみ)
  reviews?: ReturnType<typeof transformReviews>;
  threads?: ReturnType<typeof transformThreads>;
  unresolved_threads?: number;
  issue_comments?: ReturnType<typeof transformIssueComments>;
}
...
export async function cmdPrShow(
  prNumberStr: string,
  options: IssuesPrOptions & { withComments?: boolean },
  logger: Logger
): Promise<number> {
  ...
  const [result, commentsResult] = await Promise.all([
    runGraphQL<QueryResult>(GRAPHQL_QUERY_PR_SHOW, graphqlVars),
    options.withComments
      ? runGraphQL<PrCommentsQueryResult>(GRAPHQL_QUERY_PR_REVIEW_THREADS, graphqlVars)
