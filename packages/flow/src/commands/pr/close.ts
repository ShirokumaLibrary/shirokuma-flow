...
export async function cmdPrClose(
  ...
  if (prNumberStr && isIssueNumber(prNumberStr)) {
    ...
    logger.info(
      "Usage: shirokuma-flow pr close <number>\n" +
      "       shirokuma-flow pr close --head <branch>"
    );
    return 1;
  }
  ...
  const linkedIssues: { number: number; status: string; rolled_back: boolean }[] = [];

  // 情報取得を並列化（差し戻し処理は副作用があるため逐次）
  const detailResults = await Promise.allSettled(
    linkedIssueNumbers.map((n) => getIssueDetail(owner, repo, n))
  );
  ...
  for (let i = 0; i < linkedIssueNumbers.length; i++) {
    ...
    // #2439: Completed は LEGACY 値。5 値モデル移行後は Review のみが PR close 時の差し戻し対象
    // #2440: LEGACY_STATUS_VALUES.COMPLETED_LEGACY を参照して正式化
    const isRollbackTarget =
      detail?.status === LEGACY_STATUS_VALUES.COMPLETED_LEGACY || // LEGACY: "Completed" → Done
    ...
    if (options.rollback) {
      ...
      linkedIssues.push({ number: issueNumber, status: detail?.status ?? "Review", rolled_back: false });
      logger.warn(`Issue #${issueNumber} は ${detail?.status ?? "Review"} のまま（--rollback で差し戻し可能）`);
    }
  }
