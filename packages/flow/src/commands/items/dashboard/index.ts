...
export async function cmdDashboard(
  ...
  if (gitState.branch && PROTECTED_BRANCHES.includes(gitState.branch)) {
    warnings.push(
      `On protected branch "${gitState.branch}". Create a feature branch before committing.`
    );
    logger.warn(
      `Warning: On protected branch "${gitState.branch}". Create a feature branch before committing.`
    );
  }
  if (gitState.hasUncommittedChanges) {
    warnings.push(
      `${gitState.uncommittedChanges.length} uncommitted change(s) detected.`
    );
  }
 ...
 */
async function cmdDashboardTeam(
  owner: string,
  repo: string,
  limit: number,
  options: DashboardOptions,
  ...
  const allMembers = new Set<string>(Object.keys(issuesByAssignee));
