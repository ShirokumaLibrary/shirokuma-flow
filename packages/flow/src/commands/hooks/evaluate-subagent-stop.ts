 ...
 */
export function resolveMainTranscriptPath(input: SubagentStopHookInput): string | null {
 ...
 */
export async function hooksEvaluateSubagentStopCommand(): Promise<void> {
  try {
    ...
    let parsed: SubagentStopHookInput;
    try {
      parsed = JSON.parse(input) as SubagentStopHookInput;
    } catch {
      return;
    }
