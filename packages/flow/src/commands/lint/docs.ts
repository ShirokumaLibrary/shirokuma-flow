 ...
 * lint-docs コマンドハンドラ
 */
export function lintDocsCommand(options: LintDocsOptions): number {
  ...
  if (!lintDocsConfig?.enabled) {
    ...
    if (!lintDocsConfig || lintDocsConfig.enabled === undefined) {
      logger.warn("lint-docs is not enabled in config. Add 'lintDocs: { enabled: true }' to .shirokuma/config.yaml");
    }
    return 0;
  }
...
function runLintDocs(
  ...
  for (const reqConfig of config.required) {
    if ("file" in reqConfig) {
      ...
      // パターン検証
      const result = validateFilePattern(
        projectPath,
        reqConfig,
        config,
        logger
