...
interface NpmAuditJson {
  ...
  // npm audit v6 形式
  advisories?: Record<string, NpmAdvisoryEntry>;
}

interface NpmVulnerabilityEntry {
  name: string;
  severity: string;
  isDirect: boolean;
  via: Array<string | NpmViaEntry>;
...
/**
 * yarn audit JSON 出力の型（NDJSON: 各行が JSON オブジェクト）
 */
...
export function runAudit(
  pm: PackageManager,
  projectPath: string
): { output: string; error: null } | { output: null; error: string } {
 ...
 */
function parseNpmAudit(jsonOutput: string): VulnerabilityItem[] {
  let parsed: NpmAuditJson;
  try {
    parsed = JSON.parse(jsonOutput) as NpmAuditJson;
  } catch {
    return [];
  }
    ...
    for (const [, entry] of Object.entries(parsed.vulnerabilities)) {
      ...
      for (const via of entry.via ?? []) {
        if (typeof via === "string") {
          ...
          if (via.title) description = via.title;
          if (via.cwe) cveIds.push(...via.cwe);
          if (via.name) viaNames.push(via.name);
        }
      }
  ...
  if (parsed.advisories) {
    for (const [, advisory] of Object.entries(parsed.advisories)) {
      ...
      const isDev = advisory.findings.some((f) =>
        f.paths.some((p) => p.includes(">") || p.includes("dev"))
      );
 ...
 */
function parsePnpmAudit(jsonOutput: string): VulnerabilityItem[] {
  let parsed: PnpmAuditJson;
  try {
    parsed = JSON.parse(jsonOutput) as PnpmAuditJson;
  } catch {
    return [];
  }
  ...
  if (!parsed.advisories) return items;
 ...
 */
function parseYarnAudit(jsonOutput: string): VulnerabilityItem[] {
  const items: VulnerabilityItem[] = [];
  ...
  for (const line of jsonOutput.split("\n")) {
    ...
    const entry = obj as YarnAuditAdvisory;
 ...
 */
export function parseAuditResult(pm: PackageManager, jsonOutput: string): VulnerabilityItem[] {
  switch (pm) {
    case "npm":
 ...
 */
const SEVERITY_WEIGHT: Record<VulnerabilitySeverity, number> = {
  critical: 4,
  high: 3,
  moderate: 2,
  low: 1,
 ...
 */
function parseSeverityThreshold(level: string | undefined): number {
  const lower = (level ?? "high").toLowerCase();
  if (lower === "critical") return SEVERITY_WEIGHT.critical;
  if (lower === "high") return SEVERITY_WEIGHT.high;
  if (lower === "moderate" || lower === "medium") return SEVERITY_WEIGHT.moderate;
  if (lower === "low") return SEVERITY_WEIGHT.low;
 ...
 * サマリーを計算
 */
function calcSummary(vulnerabilities: VulnerabilityItem[]): SecurityAuditReport["summary"] {
...
function formatSummary(report: SecurityAuditReport): string {
  if (report.skipped) {
...
function formatTerminal(report: SecurityAuditReport): string {
  const lines: string[] = [];
 ...
 */
export async function lintSecurityCommand(options: LintSecurityOptions): Promise<number> {
  ...
  const filtered = allVulnerabilities.filter(
    (v) => SEVERITY_WEIGHT[v.severity] >= threshold
  );
  ...
  if (report.passed) {
    ...
    logger.error(
      `セキュリティ監査失敗 - critical/high ${summary.errorCount}件の脆弱性が検出されました`
    );
  } else {
    logger.warn(
      `セキュリティ監査完了 - ${summary.errorCount}件のエラー（non-strictモード）`
    );
  }
