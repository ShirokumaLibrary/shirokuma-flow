...
import type { BenchmarkData, GradingExpectation, Stats } from "./types.js";
 ...
 */
function loadRunResults(benchmarkDir: string): Map<string, RunEntry[]> {
  ...
  for (let evalIdx = 0; evalIdx < evalDirs.length; evalIdx++) {
    ...
    if (existsSync(metadataPath)) {
      try {
        ...
        if (metadata["eval_id"] !== undefined) {
          evalId = metadata["eval_id"] as number | string;
        }
      } catch {
        // フォールバック
      }
    ...
    for (const configName of configDirs) {
      ...
      for (const runDirName of runDirs) {
        ...
        try {
          grading = JSON.parse(readFileSync(gradingFile, "utf-8")) as Record<string, unknown>;
        } catch (e) {
        ...
        if (timeSeconds === 0) {
          ...
          if (existsSync(timingFile)) {
            try {
              const timingData = JSON.parse(readFileSync(timingFile, "utf-8")) as Record<string, number>;
        ...
        const rawExpectations = (grading["expectations"] as GradingExpectation[]) ?? [];
...
function aggregateResults(
  ...
  const runSummary: Record<string, Record<string, unknown>> = {};
...
export function generateBenchmark(
  ...
  return {
    ...
    runs,
    run_summary: runSummary as BenchmarkData["run_summary"],
    notes: [],
  };
}
 ...
 */
export function generateMarkdown(benchmark: BenchmarkData): string {
  ...
  const summaryA = run_summary[configA] as unknown as Record<string, Stats> | undefined;
  const summaryB = run_summary[configB] as unknown as Record<string, Stats> | undefined;
  const delta = run_summary["delta"] as unknown as Record<string, string> | undefined;
...
export function cmdSkillBenchmark(options: BenchmarkOptions): number {
  const {
    benchmarkDir: rawBenchmarkDir,
    skillName = "",
    skillPath = "",
    output,
    verbose,
  ...
  const delta = benchmark.run_summary["delta"] as unknown as Record<string, string> | undefined;
  process.stdout.write("\nSummary:\n");
  for (const config of configs) {
    const summary = benchmark.run_summary[config] as unknown as Record<string, Stats> | undefined;
