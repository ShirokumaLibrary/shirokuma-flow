...
import type { EvalEntry, EvalQueryResult, EvalResult } from "./types.js";
          ...
          if (eventType === "assistant") {
            ...
            if (content) {
              for (const item of content) {
                ...
                if (ci["type"] !== "tool_use") continue;
                const toolName = ci["name"] as string;
                const toolInput = ci["input"] as Record<string, unknown> | undefined;
                if (toolName === "Skill" && (toolInput?.["skill"] as string)?.includes(cleanName)) {
                  triggered = true;
                } else if (toolName === "Read" && (toolInput?.["file_path"] as string)?.includes(cleanName)) {
                  triggered = true;
                }
              }
            }
...
export async function runEval(
  ...
  const allTasks: Array<{ item: EvalEntry; runIdx: number }> = [];
  ...
  const queryItems = new Map<string, EvalEntry>();
...
export async function cmdSkillEval(options: EvalOptions): Promise<number> {
  const {
