...
export function parseManifest(content: string): ManifestEntry[] {
  ...
  for (const line of content.split('\n')) {
    ...
    if (!source || !pkg || !date || !count) continue;
    entries.push({
      source,
      package: pkg,
      lastFetched: date,
...
export function formatManifest(entries: readonly ManifestEntry[]): string {
  ...
  for (const entry of [...entries].sort((a, b) => a.source.localeCompare(b.source))) {
    lines.push(
      `| ${entry.source} | ${entry.package} | ${entry.lastFetched} | ${entry.fileCount} |`,
    );
  }
  lines.push('');
  return lines.join('\n');
...
export interface WriteManifestParams {
  projectPath: string;
  sources: readonly { name: string; outputDir?: string }[];
  docsRoot?: string;
  /**
 ...
 */
export async function writeManifest(params: WriteManifestParams): Promise<void> {
  ...
  for (const src of params.sources) {
    ...
    if (params.resolvePackageName) {
      try {
        ...
        if (resolved) packageName = resolved;
      } catch {
  ...
  const survivingExisting = existingEntries.filter((e) => {
    if (newSourceNames.has(e.source)) return false;
    const outDir = resolveOutputDir({
      projectPath: params.projectPath,
      sourceName: e.source,
      docsRoot: params.docsRoot,
...
function resolveContextsRoot(projectPath: string, docsRoot?: string): string {
  return docsRoot ? resolve(projectPath, docsRoot) : resolve(projectPath, DEFAULT_CONTEXTS_ROOT);
}

function readExistingManifest(manifestPath: string): ManifestEntry[] {
  if (!existsSync(manifestPath)) return [];
  try {
