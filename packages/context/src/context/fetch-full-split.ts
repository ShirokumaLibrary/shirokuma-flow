...
export async function fetchFullSplit(params: {
  ...
  try {
    ...
    logger.error(`[${src.name}] llms-full.txt の取得に失敗: ${(err as Error).message}`);
    return stats;
  }
  ...
  try {
    ...
    logger.error(
      `[${src.name}] 無効な splitPattern です: "${splitPatternStr}" — ${(err as Error).message}`,
    );
    return stats;
  }
...
function splitSections(content: string, pattern: RegExp): string[] {
