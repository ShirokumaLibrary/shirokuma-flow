...
export function transformMdxToMd(content: string): string {
  let result = content;
...
function extractExportConst(content: string, name: string): string | null {
...
function buildHeader(title: string | null, description: string | null): string {
  let header = '';
...
function removeExportConsts(content: string): string {
  // 単一行
...
function transformApiTables(content: string): string {
  // <ApiTable\n  rows={[...]}  /> のブロック全体を検出
  // 複数行にまたがる JSX ブロックをマッチ
  return content.replace(/<ApiTable\s[\s\S]*?\/>/g, (match) => parseApiTableToMarkdown(match));
}

function parseApiTableToMarkdown(block: string): string {
  // rows={[ の開始を探す
 ...
 */
function parseArrayRows(arrayStr: string): string[][] {
  const rows: string[][] = [];
  // 外側の [ ] を除去
 ...
 */
function parseRowCells(rowStr: string): string[] | null {
...
function extractQuotedString(text: string, start: number): { value: string; end: number } | null {
 ...
 */
function findJsxElementEnd(lines: string[], start: number): number {
  const startTrimmed = (lines[start] ?? '').trim();

  // 単一行で自己閉じ
  if (/\/>\s*$/.test(startTrimmed)) return start;
  ...
  for (let j = start + 1; j < lines.length && j < start + 50; j++) {
    ...
    // 自己閉じ
    if (/\/>\s*$/.test(lt)) return j;
    // 対応する閉じタグ
    if (tagName && lt === `</${tagName}>`) return j;
    // > で開きタグが閉じた後、内容を含んで閉じタグ
    if (tagName && new RegExp(`</${tagName}>`).test(lt)) return j;
  }
...
function buildMarkdownTable(headers: string[], rows: string[][]): string {
  ...
  const dataLines = rows.map(
    (row) => `| ${row.slice(0, headers.length).map(escapeCell).join(' | ')} |`,
  );
...
function transformContentComponents(content: string): string {
  let result = content;
  ...
  // JSX 式コンテナ {<>...</>} を含む場合もある
  const tipPatterns: [RegExp, string][] = [
...
function removeJsxWrappers(content: string): string {
  const wrapperTags = [
    'Figure',
    'Example',
    'Stripes',
...
function removeSelfClosingJsx(content: string): string {
  ...
  while (i < lines.length) {
    ...
    if (trimmed === '{') {
      ...
      while (j < lines.length && depth > 0) {
        ...
        if (depth > 0 && /^<[a-z]/i.test(lt)) hasJsx = true;
        if (depth > 0 && /className=/.test(lt)) hasJsx = true;
        j++;
      }
      if (hasJsx && depth === 0) {
        i = j;
    ...
    if (/^\s*\{<[a-z]/i.test(line) && /className=/.test(line)) {
      // 単一行で閉じている場合
      if (/\}\s*$/.test(trimmed)) {
        i++;
        continue;
      ...
      while (j < lines.length) {
        const lj = lines[j] ?? '';
        if (/^\s*\}\s*$/.test(lj.trim()) || lj.includes('}')) break;
        j++;
      }
      i = j + 1;
      continue;
    ...
    // 複数行: <img\n  className=...\n  src=...\n/>
    if (/^\s*<[a-z][\w-]*[\s>]/.test(line) && !trimmed.startsWith('<http')) {
    ...
    if (/^\s+className=/.test(line) || /^\s+src=/.test(line)) {
      i++;
      continue;
    }
