...
export async function execute(params: PresetExecuteParams): Promise<FetchStats> {
  ...
  if (!repoInfo) {
    logger.error(
      `[${src.name}] typescript-5 プリセットには url を "https://github.com/{owner}/{repo}" 形式で指定してください。`,
    );
    return stats;
  }
  ...
  const filesToFetch: { path: string; outSubdir: string; relativePath: string }[] = [];
  for (const pkg of DOC_PACKAGES) {
    const pkgFiles = tree.filter(
      (item) =>
        item.type === 'blob' && item.path.endsWith('.md') && item.path.startsWith(pkg.prefix),
    );
