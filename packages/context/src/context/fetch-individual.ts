...
export async function fetchIndividual(params: {
  ...
  if (options.dryRun) {
    ...
    for (const url of resolvedUrls) logger.info(`  ${url}`);
    return stats;
  }
...
function adaptLinkFormat(url: string, linkFormat: 'md' | 'clean'): string {
  if (linkFormat !== 'clean' || url.endsWith('.md')) return url;
...
function applyStripPatterns(
  outFile: string,
  stripHeader: RegExp | null,
  stripLine: RegExp | null,
  sourceName: string,
  filename: string,
  logger: Logger,
): void {
  try {
    ...
    if (stripHeader) content = content.replace(stripHeader, '');
    if (stripLine) {
      content = content
        .split('\n')
    ...
    logger.debug?.(`[${sourceName}] パターン除去でエラー: ${filename} — ${(err as Error).message}`);
  }
}
