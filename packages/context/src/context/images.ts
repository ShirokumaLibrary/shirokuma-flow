...
export function extractImageUrls(content: string): string[] {
  ...
  const emit = (raw: string | undefined): void => {
    if (!raw || !HTTP_PREFIX.test(raw)) return;
    const canonical = raw.split('?')[0] ?? raw;
    if (seen.has(canonical) || !hasExtension(canonical)) return;
    seen.add(canonical);
    urls.push(raw);
  };

  const imgRegex = /!\[[^\]]*\]\((https?:\/\/[^)\s]+)\)/g;
  for (const match of content.matchAll(imgRegex)) emit(match[1]);

  const srcRegex = /<img\s[^>]*src=["']([^"']+)["']/gi;
  for (const match of content.matchAll(srcRegex)) emit(match[1]);
 ...
 */
export function rewriteImagePaths(content: string, rewrites: Map<string, string>): string {
  let result = content;
