...
function writeError(message: string, extra: Record<string, unknown>, pretty: boolean): void {
  emitJson({ error: message, ...extra }, { exitCode: 1, pretty });
}

function readProjectPackageJson(projectPath: string, pretty: boolean): Record<string, unknown> {
  ...
  try {
    ...
    if (code === 'ENOENT') {
      writeError(
        ...
        { path: pkgJsonPath, hint: 'Node.js プロジェクトのルートで実行してください' },
        pretty,
      );
    } else {
...
async function regenerateManifest(projectPath: string, docsRoot: string | undefined): Promise<void> {
  try {
...
export function registerContextCommands(parent: Command): void {
  parent
    .description(
      '外部ドキュメント（llms.txt / GitHub）を取得してローカルに Markdown として保持する CLI。',
    )
  ...
  function contextFromParent(): CliContext {
    const opts = parent.opts<GlobalOpts>();
  ...
  parent
    ...
    .action((name: string, cmdOpts: { yes?: boolean }) => {
      const ctx = contextFromParent();
  ...
  parent
    .command('fetch [name]')
    .description(
      '指定プリセットでドキュメントを取得する。name 省略時は fetch 済みソースを全件再取得。',
    )
    ...
    .option(
      '--auto-detect',
      'package.json の依存を解析し、プリセットにマッチする未 fetch ソースをまとめて取得',
    )
    .action(
      async (
        name: string | undefined,
        cmdOpts: { force?: boolean; dryRun?: boolean; images?: boolean; autoDetect?: boolean },
      ) => {
        const ctx = contextFromParent();
        ...
        if (cmdOpts.autoDetect && name) {
          writeError(
            '--auto-detect と name 引数は同時に指定できません',
            { hint: 'name 指定: fetch <name>  /  自動検出: fetch --auto-detect' },
            ctx.pretty,
          );
        }
        ...
        if (cmdOpts.autoDetect) {
          ...
          const results: Array<{ name: string; outDir: string; stats: unknown }> = [];
          let anyFailed = false;
        ...
        if (allSources.length === 0) {
          writeError(
            '取得済みのドキュメントがありません',
            { hint: '取得するには: shirokuma-context fetch <name>' },
            ctx.pretty,
          );
        }
        ...
        for (const src of allSources) {
          ...
          if (stats.failed > 0) anyFailed = true;
          if (!cmdOpts.dryRun) results.push({ name: src.name, outDir, stats });
        }
  ...
  parent
    .command('detect')
    .description(
      'package.json の依存関係からドキュメントソースを自動検出（プリセット逆引き）',
    )
    .option(
      '--format <format>',
      '出力形式: table-json, json（既定: table-json）',
      'table-json',
    )
    .action((cmdOpts: { format?: string }) => {
      const ctx = contextFromParent();
      const config = loadContextsConfig(ctx.projectPath);
      const docsRoot = ctx.docsRoot ?? config.contexts?.outputDir;
      ...
      if (detected.length === 0) {
        writeJson(
          {
            message:
              'ビルトインプリセットにマッチする依存関係が見つかりませんでした。fetch --help でプリセット一覧を確認してください。',
            detected: [],
          },
          ctx.pretty,
        );
