...
} as const satisfies Record<string, AnyPresetMeta>;

export type PresetName = keyof typeof PRESETS;

/** レジストリに登録されているプリセット名の一覧（ソート済み）。 */
export function listPresetNames(): PresetName[] {
  return (Object.keys(PRESETS) as PresetName[]).sort();
}
 ...
 */
export function resolvePresetMeta<N extends PresetName>(name: N): (typeof PRESETS)[N];
export function resolvePresetMeta(name: string): AnyPresetMeta | null;
export function resolvePresetMeta(name: string): AnyPresetMeta | null {
  return (PRESETS as Record<string, AnyPresetMeta>)[name] ?? null;
}
