...
export async function execute(params) {
    ...
    const fetchTasks = pageTasks.map((task) => async () => {
        if (liveFailures >= FAILURE_THRESHOLD)
            return 'failed';
 ...
 *   `- [title](*.html)` でページエントリ
 */
export function parseLlmsTxtSections(content) {
    const sections = [];
    let current = null;
    for (const line of content.split('\n')) {
        const trimmed = line.trim();
        const sectionMatch = trimmed.match(/^##\s+\[([^\]]+)\]\(([^)]+)\)/) ?? trimmed.match(/^##\s+(.+)/);
        if (sectionMatch?.[1]) {
            if (current)
 ...
 */
export function htmlToMarkdown(html, td) {
    let content = html;
    const mainPatterns = [
 ...
 */
async function remoteIsNewerThanLocal(url, outFile) {
    let localMtime;
    try {
