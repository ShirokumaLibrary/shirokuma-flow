...
function formatSectionWithMetadata(section, titleMap) {
    ...
    const fm = ['---'];
    if (title)
        fm.push(`title: ${yamlDoubleQuote(title)}`);
 ...
 */
export function deriveSectionFilename(section, index, usedNames) {
...
function pickBase(section) {
    const titleMatch = section.match(/^title:\s*"?([^"\n]+)"?$/m);
    if (titleMatch?.[1])
        return slugify(titleMatch[1]) || null;
    const h1Match = section.match(/^# (.+)$/m);
    if (h1Match?.[1])
        return slugify(h1Match[1]) || null;
...
//# sourceMappingURL=section-format.js.map