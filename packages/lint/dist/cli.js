...
import { commandToJson, emitJson, errorToJson, isBenignCommanderError, preflightArgv, walkCommands, } from './help-json.js';
...
function parseJsonOrExit(json, label, pretty) {
    try {
        return JSON.parse(json);
    }
    catch (err) {
        emitJson({ error: `invalid ${label} JSON: ${err instanceof Error ? err.message : String(err)}` }, { exitCode: 1, pretty });
    }
}
function parseConfigFileOrExit(filePath, label, pretty) {
    ...
    try {
        ...
        emitJson({ error: `${label}: unsupported extension "${ext}" (use .yaml / .yml / .json)` }, { exitCode: 1, pretty });
    }
    catch (err) {
        emitJson({
 ...
 */
function loadConfigOrExit(inline, file, flag, pretty) {
    const inlineLabel = `--${flag}`;
    const fileLabel = `--${flag}-file`;
    if (inline !== undefined && file !== undefined) {
        emitJson({ error: `${inlineLabel} and ${fileLabel} are mutually exclusive` }, { exitCode: 1, pretty });
    }
    if (inline !== undefined)
        return parseJsonOrExit(inline, inlineLabel, pretty);
    if (file !== undefined)
        return parseConfigFileOrExit(file, fileLabel, pretty);
...
program
    ...
    .option('--project-root <path>', 'monorepo root（`.shirokuma/` を持つディレクトリ）。未指定時は --project から walk up で auto-discover (Issue #49)')
...
program
    ...
    .option('--config <json>', 'LintDocsConfig JSON（例: \'{"required":[{"file":"README.md","description":"..."}]}\'）')
    .option('--config-file <path>', 'LintDocsConfig を格納した YAML / JSON ファイルパス')
    .action((cmdOpts) => {
    const opts = program.opts();
    const projectPath = resolveProject(opts.project);
    const parsed = loadConfigOrExit(cmdOpts.config, cmdOpts.configFile, 'config', pretty);
...
program
    ...
    .option('--config <json>', 'LintCodeConfig JSON（例: \'{"rules":[{"filePattern":"src/**/*.ts","functionTags":["@returns"]}]}\'）')
...
program
    ...
    .option('--config <json>', 'LintCommitFormatConfig JSON（例: \'{"commits":[{"hash":"abc","subject":"feat: x"}]}\'）')
...
program
    .command('all')
    .description('coverage + (structure は --project-root 配下 .shirokuma/lint/structure.yaml を auto-discover、 docs|code|commit-format は --config / --config-file 指定時のみ) を一括実行')
    ...
    .option('--structure-config-file <path>', 'structure 用 LintStructureConfig を格納した YAML / JSON ファイル')
    ...
    .option('--commit-format-config-file <path>', 'commit-format 用 LintCommitFormatConfig を格納した YAML / JSON ファイル')
    .action((cmdOpts) => {
    const opts = program.opts();
    const projectPath = resolveProject(opts.project);
    let projectRoot;
    if (opts.projectRoot !== undefined) {
        ...
        if (!existsSync(projectRoot) || !statSync(projectRoot).isDirectory()) {
            emitJson({
                ...
                help_hint: 'pass an existing directory that contains .shirokuma/, or omit to auto-discover',
            }, { exitCode: 1, pretty });
        }
    }
    else {
    ...
    function runIfConfigured(resultKey, flag, ruleName, inline, file, run) {
        let effectiveFile = file;
        if (inline === undefined && effectiveFile === undefined) {
            if (!AUTO_DISCOVER_RULES.has(ruleName))
                return;
        ...
        const parsed = loadConfigOrExit(inline, effectiveFile, flag, pretty);
    ...
    if (!coverageReport.passed)
        anyFailed = true;
    runIfConfigured('docs', 'docs-config', 'docs', cmdOpts.docsConfig, cmdOpts.docsConfigFile, (config) => lintDocs({ projectPath, config }));
    runIfConfigured('structure', 'structure-config', 'structure', cmdOpts.structureConfig, cmdOpts.structureConfigFile, (config) => lintStructure({ projectPath, config }));
    runIfConfigured('code', 'code-config', 'code', cmdOpts.codeConfig, cmdOpts.codeConfigFile, (config) => lintCode({ projectPath, config }));
    runIfConfigured('commitFormat', 'commit-format-config', 'commit-format', cmdOpts.commitFormatConfig, cmdOpts.commitFormatConfigFile, (config) => lintCommitFormat({ config }));
