...
import { checkDocumentLength, checkFileExists, checkFrontmatter, checkInternalLinks, checkSections, mergeResults, } from './markdown-structure.js';
import { isFileConfig, isPatternConfig } from './docs-types.js';
export function lintDocs(params) {
...
function validateArtifactCrossRef(projectPath, config) {
    const include = config.include && config.include.length > 0
        ? config.include
        : [
            '.claude/skills/**/*.md',
            '.claude/agents/*.md',
...
function validateCompactTables(projectPath, config) {
...
function validateSingleFile(projectPath, fileConfig, lintConfig) {
    ...
    if (fileConfig.minLength !== undefined || fileConfig.maxLength !== undefined) {
        results.push(checkDocumentLength(content, { minLength: fileConfig.minLength, maxLength: fileConfig.maxLength }, filePath));
    }
...
function validateFilePattern(projectPath, patternConfig, lintConfig) {
