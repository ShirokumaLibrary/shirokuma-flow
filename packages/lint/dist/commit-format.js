...
export function lintCommitFormat(params) {
    const { commits, allowedTypes = DEFAULT_TYPES, maxSubjectLength = DEFAULT_MAX_SUBJECT_LENGTH, severity = 'warning', } = params.config;
    const issues = [];
    for (const commit of commits) {
        if (isSkippable(commit.subject))
            continue;
