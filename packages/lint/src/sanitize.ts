...
export function validateProjectPath(inputPath: string): string {
  ...
  if (resolved === '/') {
    throw new Error(
      `Project path "${inputPath}" resolves to root directory "/". ` +
  ...
  for (const prefix of BLOCKED_SYSTEM_PREFIXES) {
    if (isUnder(resolved, prefix)) {
      throw new Error(
        `Project path "${inputPath}" resolves to system directory "${resolved}". ` +
  ...
  throw new Error(
    `Project path "${inputPath}" resolves to "${resolved}" which is outside ` +
