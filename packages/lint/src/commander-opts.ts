...
export function mergeCommanderOpts(command: MergeableCommand, localOpts: Record<string, unknown>): any {
  ...
  const ancestors: MergeableCommand[] = [];
  let cursor: MergeableCommand | null | undefined = command.parent;
  while (cursor) {
  ...
  const ancestorOpts: Record<string, unknown> = {};
  for (let i = ancestors.length - 1; i >= 0; i--) {
    Object.assign(ancestorOpts, ancestors[i].opts());
  }

  const merged: Record<string, unknown> = { ...ancestorOpts, ...localOpts };
  ...
  for (const key of Object.keys(localOpts)) {
    ...
    if (childSource !== "default") continue;
