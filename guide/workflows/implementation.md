...
- **設計が必要**: `create-item-flow` → `/design-flow` → `/prepare-flow` → `/implement-flow`
- **設計不要 + サイズ M+**: `create-item-flow` → `/prepare-flow` → `/implement-flow`
- **設計不要 + サイズ XS/S かつ要件明確**: `create-item-flow` → `/implement-flow`（直接）

## 自動フローの全体像

```
## 起点
/create-item-flow              Issue 作成 → 自動で次へ
...
- `create-item-flow` で Issue を作成すると、自動的に `implement-flow` にチェーンする
