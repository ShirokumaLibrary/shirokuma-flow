...
shirokuma-flow は複数のバイナリに分かれている。インストール対象（`install.sh` / `npm i -g @shirokuma-library/flow`）は `shirokuma-flow` のみで、生成系・lint 系のサブパッケージは個別に追加する。

| バイナリ | パッケージ | 役割 |
|---------|-----------|------|
| `shirokuma-flow` | `@shirokuma-library/flow` | GitHub Issues / PR / Discussions / Projects 管理、init、update、sync-github |
