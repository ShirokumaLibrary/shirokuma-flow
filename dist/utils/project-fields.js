...
import { validateStatusTransition, STATUS_ORDER } from "./status-workflow.js";
/**
 ...
 */
export async function setItemFields(projectId, itemId, fields, logger, cachedFields, currentStatus, force) {
    if (Object.keys(fields).length === 0)
        return 0;
    const projectFields = cachedFields ?? await getProjectFields(projectId);
    let updatedCount = 0;
    const failedFields = [];
    // Status transition validation (#382, #1935: block by default)
    if (currentStatus && fields["Status"]) {
        ...
        if (!validation.valid) {
            if (!force) {
                ...
                logger?.info("強制実行するには --force オプションを使用してください");
                return 0;
            }
            logger?.warn(`${validation.warning ?? ""} (--force で続行)`);
        }
    }
 ...
 */
export async function autoSetTimestamps(projectId, itemId, statusValue, projectFields, logger, timestamp, previousStatus) {
