 ...
 */
export function getPluginCacheBaseRoot() {
    ...
    if (process.env.NODE_ENV === "test" && !warnedTestCacheUnset) {
        ...
        console.warn(`[shirokuma-docs] 警告: NODE_ENV=test ですが SHIROKUMA_PLUGIN_CACHE_ROOT が未設定です。実ユーザーのキャッシュ (~/.claude/plugins/cache/) への書き込み/削除が発生します。`);
    }
 ...
 */
export function deployRules(projectPath, options = {}) {
    const logger = createLogger(options.verbose ?? false);
 ...
 */
export function getGlobalCachePath(pluginName, version, options) {
    ...
    try {
        const versions = readdirSync(cacheBase)
            .filter(name => {
            if (name === "local")
                return false; // local/ は semver ではない
            try {
 ...
 */
export function detectInstalledOptionalPlugins(projectPath) {
 ...
 */
function copyPluginFromMarketplace(registryId, reinstall) {
 ...
 */
export function cleanDeployedRules(projectPath, options = {}) {
    const logger = createLogger(options.verbose ?? false);
...
export function cleanupOldCacheVersions(pluginName, keepCount = 3) {
    ...
    try {
        const versions = readdirSync(cacheBase)
            .filter(name => {
            if (name === "local")
                return false; // local/ は semver ではない (#1411)
            try {
                return statSync(join(cacheBase, name)).isDirectory();
            }
            catch {
 ...
 */
export function cleanupLocalCacheDir(pluginName) {
 ...
 * @returns インストール結果
 */
export function installLocalPlugin(pluginName, sourcePath, options = {}) {
...
export async function installAllPlugins(options) {
    ...
    if (cleanupOldVersions) {
        ...
        for (const pn of pluginNames) {
            // local/ を削除（update-skills --sync はリリース版への切り戻しを意図する）
            if (cleanupLocalCacheDir(pn)) {
                console.warn(`[shirokuma-docs] dev モードプラグイン検出: ${pn}/local/ を削除しました。update-skills --sync によりリリース版に切り戻しました。`);
