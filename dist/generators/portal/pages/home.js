    ...
    return renderTemplate("pages/home.html.hbs", {
        ...
        featureMap: data.featureMap
            ? {
                ...
                screenCount: Object.values(data.featureMap.features).reduce((sum, g) => sum + (g.screens?.length || 0), 0),
                componentCount: Object.values(data.featureMap.features).reduce((sum, g) => sum + (g.components?.length || 0), 0),
                actionCount: Object.values(data.featureMap.features).reduce((sum, g) => sum + (g.actions?.length || 0), 0),
            }
            : null,
...
function buildStats(data) {
    ...
    if (data.available.hasFeatureMap && data.featureMap) {
        const total = Object.values(data.featureMap.features).reduce((sum, g) => sum +
            (g.screens?.length || 0) +
            (g.components?.length || 0) +
            (g.actions?.length || 0), 0);
        stats.push({
            label: "機能アイテム",
            value: total,
            unit: "件",
