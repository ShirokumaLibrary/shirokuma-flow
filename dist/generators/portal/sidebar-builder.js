 ...
 */
export function buildSidebarData(data) {
    const sections = [];
    // ホームセクション
    sections.push({
        title: "メイン",
    ...
    if (data.available.hasFeatureMap) {
        const featureCount = data.featureMap
            ? Object.values(data.featureMap.features).reduce((sum, g) => sum +
                (g.screens?.length || 0) +
    ...
    // アプリセクション
    if (data.available.hasApplications &&
        data.applications?.apps &&
        data.applications.apps.length > 0) {
        const appLinks = data.applications.apps.map((app) => ({
            label: app.name,
