...
export class Extractor {
    ...
    applyMapping(value, mappingName, fieldName, result) {
        ...
        // No mapping found
        result.warnings.push(`Unmapped value: ${fieldName} = "${stringValue}" (not found in ${mappingName})`);
        return value;
    }
    /**
        ...
        if (this.config.extraction?.templates?.[documentType]) {
            ...
            for (const [key, value] of Object.entries(template)) {
                if (key === 'depends_on' && Array.isArray(value)) {
                    ...
                    frontmatter[key] = value.map((dep) => typeof dep === 'string' ? this.substituteVariables(dep, extractedFields) : dep);
                }
                else {
