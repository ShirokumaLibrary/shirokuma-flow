...
export async function execute(src, outDir, options, stats, logger) {
    const repoInfo = resolveGithubRepo(src, "cakephp-5", logger);
    if (!repoInfo)
        return stats;
    const { owner, repo } = repoInfo;
    const branch = src.branch ?? "5.x";
    const repoPath = src.repoPath ?? "docs/en";
    ...
    const mdFiles = tree.filter((item) => {
        if (item.type !== "blob")
            return false;
        if (!item.path.endsWith(".md") && !item.path.endsWith(".rst"))
            return false;
        if (repoPath) {
    ...
    logger.info(`[${src.name}] ${mdFiles.length} 件のドキュメントファイルを取得します。`);
    if (options.dryRun) {
...
//# sourceMappingURL=cakephp-5.js.map