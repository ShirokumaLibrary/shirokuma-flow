...
/** Java/Python/C# 向けの言語固有ファイルを除外するパターン */
const EXCLUDE_LANG_SUFFIX = /-(java|python|csharp)\.md$/;
/**
 * プリセットエントリーポイント。
 * fetchSource() から動的 import で呼び出される。
 */
export async function execute(src, outDir, options, stats, logger) {
    const repoInfo = resolveGithubRepo(src, "playwright-1", logger);
    if (!repoInfo)
        return stats;
    ...
    // repoPath 配下の .md ファイルのみフィルタ（Java/Python/C# 言語固有ファイルを除外）
    const mdFiles = tree.filter((item) => {
        if (item.type !== "blob")
            return false;
        if (!item.path.endsWith(".md"))
            return false;
        if (EXCLUDE_LANG_SUFFIX.test(item.path))
            return false;
        if (repoPath) {
    ...
    logger.info(`[${src.name}] ${mdFiles.length} 件の Markdown ファイルを取得します（言語固有ファイル除外済み）。`);
    if (options.dryRun) {
