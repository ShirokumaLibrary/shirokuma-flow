...
/**
 * laravel-12 プリセットのメタ情報。
 * `resolvePresetMeta("laravel-12")` が動的 import でこれを取得する。
 */
export const meta = {
...
export async function execute(src, outDir, options, stats, logger) {
    const repoInfo = resolveGithubRepo(src, "laravel-12", logger);
    if (!repoInfo)
        return stats;
    const { owner, repo } = repoInfo;
    const branch = src.branch ?? "12.x";
