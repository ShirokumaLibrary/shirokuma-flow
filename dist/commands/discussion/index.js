...
export function createDiscussionCommand() {
    const discussion = new Command("discussion")
        .description("GitHub Discussions / ADR 管理 (categories, list, show, search, templates, adr)");
    // 共通親オプション
    discussion
    ...
    adr
        ...
        .action(async (title, _localOpts, command) => {
        const adrOpts = command.parent?.opts() ?? {};
        const discussionOpts = command.parent?.parent?.opts() ?? {};
        const options = { ...discussionOpts, ...adrOpts };
    ...
    adr
        ...
        const adrOpts = command.parent?.opts() ?? {};
        const discussionOpts = command.parent?.parent?.opts() ?? {};
        const options = { ...discussionOpts, ...adrOpts, ...localOpts };
    ...
    adr
        ...
        .action(async (target, _localOpts, command) => {
        const adrOpts = command.parent?.opts() ?? {};
        const discussionOpts = command.parent?.parent?.opts() ?? {};
        const options = { ...discussionOpts, ...adrOpts };
