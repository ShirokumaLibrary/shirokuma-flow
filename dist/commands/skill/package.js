 ...
 * @returns 除外する場合 true
 */
function shouldExclude(fullPath, skillPath) {
    ...
    if (parts.length >= 1 && ROOT_EXCLUDE_DIRS.has(parts[0]))
        return true;
    // ファイル名とパス
    ...
    if (dotIdx !== -1 && EXCLUDE_EXTS.has(fileName.slice(dotIdx)))
        return true;
    return false;
}
/**
 ...
 */
function collectFiles(dirPath, skillPath) {
    const files = [];
    const entries = readdirSync(dirPath, { withFileTypes: true });
    for (const entry of entries) {
        const fullPath = join(dirPath, entry.name);
        if (shouldExclude(fullPath, skillPath))
            continue;
...
export function cmdSkillPackage(options) {
    ...
    const result = spawnSync("zip", ["-r", outputFile, ...relativePaths], {
        cwd: parentDir,
        encoding: "utf-8",
