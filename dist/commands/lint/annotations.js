 ...
 */
export function lintAnnotationsCommand(options) {
 ...
 * アノテーションを自動修正
 */
function runFixAnnotations(projectPath, config, logger) {
    const results = [];
 ...
 */
function runLintAnnotations(projectPath, config, logger) {
    const fileResults = [];
    // Screen files (page.tsx)
    const screenRule = config.rules["screen-required"];
    if (screenRule) {
