 ...
 */
export async function pushComment(owner, repo, number, commentIdStr, options, logger) {
    ...
    if (parentType === "discussion" || isDiscussionComment) {
        ...
        const issueOrPrType = commentType === "pull_request" ? "pull_request" : "issue";
...
export async function pushIssueComment(owner, repo, number, databaseId, localBody, localMeta, _options, logger, baseDir = GITHUB_CACHE_DIR, type = "issue") {
...
export async function pushDiscussionComment(owner, repo, number, databaseId, localBody, localMeta, _options, logger, baseDir = GITHUB_CACHE_DIR) {
    // コメント本文の不正 Unicode 検証
    const bodyError = validateBody(localBody);
    if (bodyError) {
        logger.error(bodyError);
...
`;
    const commentIdResult = await runGraphQL(QUERY_DISCUSSION_COMMENT_ID, {
        owner,
        name: repo,
        number,
    });
