 ...
 */
function inferPrefixFromIssueType(issueType) {
    if (!issueType)
        return "feat";
    const lower = issueType.toLowerCase();
    if (lower === "bug")
        return "fix";
    if (lower === "task" || lower === "chore")
        return "chore";
    if (lower === "epic")
        return "epic";
    if (lower === "feature")
 ...
 */
function extractIntegrationBranch(body) {
 ...
 */
export async function findIntegrationBranch(parentNumber, parentBody) {
    // 1. 親 Issue 本文から抽出
    if (parentBody) {
 ...
 */
async function createIntegrationBranch(owner, repo, parentNumber, parentTitle, parentIssueId, logger) {
...
`;
    const bodyResult = await runGraphQL(GRAPHQL_QUERY_PARENT_BODY, { id: parentIssueId });
    if (bodyResult.success) {
        const currentBody = bodyResult.data?.data?.node?.body ?? "";
        const integrationSection = `\n\n### Integration Branch\n\n\`${branchName}\`\n`;
        const newBody = currentBody + integrationSection;
        await runGraphQL(GRAPHQL_MUTATION_UPDATE_ISSUE_BODY, {
            id: parentIssueId,
            body: newBody,
        });
    }
 ...
 * Issue 番号を起点にフィーチャーブランチを作成する。
 */
export async function cmdItemBranch(numberStr, options, logger) {
    ...
    // Issue 情報を取得
    const issueResult = await runGraphQL(GRAPHQL_QUERY_ISSUE_FOR_BRANCH, { owner, name: repo, number });
    ...
    if (issue.parent?.number) {
        const integrationBranch = await findIntegrationBranch(issue.parent.number, issue.parent.body);
        if (integrationBranch) {
            if (options.base) {
                logger.warn(`Integration ブランチ \`${integrationBranch}\` が存在しますが --base の指定を優先します（${options.base}）`);
            }
            else {
...
`;
                    const parentIdResult = await runGraphQL(GRAPHQL_QUERY_PARENT_ID, { owner, name: repo, number: issue.parent.number });
                    const parentIssueId = parentIdResult.success
                        ? (parentIdResult.data?.data?.repository?.issue?.id ?? "")
                        : "";
                    const newIntegrationBranch = await createIntegrationBranch(owner, repo, issue.parent.number, issue.parent.title ?? "", parentIssueId, logger);
