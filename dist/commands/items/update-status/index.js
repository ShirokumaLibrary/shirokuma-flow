 ...
 */
import { runGraphQL, getRepoInfo, isIssueNumber, parseIssueNumber, } from "../../../utils/github.js";
...
import { fetchActiveIssues, updateIssueStatus, findMergedPrForIssue, isIssueClosed, getGitState, } from "../shared/session-utils.js";
import { closeIssueById, fetchPrsWithProjectStatus } from "../integrity/index.js";
 ...
 */
export async function cmdUpdateStatus(options, logger) {
    ...
    if (git.hasUncommittedChanges) {
        warnings.push(`${git.uncommittedChanges.length} uncommitted change(s) detected. Consider committing or stashing before updating status.`);
    ...
    const updatedIssues = [];
    const updatedPrNumbers = new Set();
    ...
    if (doneNumbers.length > 0 || reviewNumbers.length > 0) {
        ...
        if (notClosedMissingNumbers.length > 0) {
            ...
            for (const pr of prs) {
                if (notClosedMissingNumbers.includes(pr.number) &&
                    pr.projectItemId &&
                    pr.projectId) {
        ...
        for (const num of doneNumbers) {
            ...
            if (!issue?.projectItemId || !issue?.projectId) {
                ...
                if (prFallback) {
                    ...
                    if (await updateIssueStatus(prFallback.projectId, prFallback.projectItemId, "Done", fields, logger, prFallback.status ?? undefined)) {
            ...
            if (await updateIssueStatus(issue.projectId, issue.projectItemId, "Done", fields, logger, issue.status ?? undefined)) {
                updatedIssues.push({ number: num, status: "Done" });
        ...
        for (const num of reviewNumbers) {
            ...
            if (!issue?.projectItemId || !issue?.projectId) {
                ...
                if (prFallback) {
                    ...
                    if (await updateIssueStatus(prFallback.projectId, prFallback.projectItemId, "Review", fields, logger, prFallback.status ?? undefined)) {
                        updatedIssues.push({ number: num, status: "Review" });
                        updatedPrNumbers.add(num);
        ...
        for (const updated of updatedIssues) {
            if (updatedPrNumbers.has(updated.number))
                continue;
