 ...
 */
import { runGraphQL, getRepoInfo, parseIssueNumber, } from "../../../utils/github.js";
import { detectCurrentRepoPair, parseRepoFullName, } from "../../../utils/repo-pairs.js";
import { GRAPHQL_MUTATION_ADD_COMMENT, GRAPHQL_MUTATION_CREATE_ISSUE, getRepoId, } from "../../../utils/graphql-queries.js";
...
import { GRAPHQL_QUERY_ISSUE_DETAIL as ISSUE_DETAIL_QUERY, setFieldsWithStatusRouting, } from "../../../utils/issue-detail.js";
 ...
 */
export async function cmdImport(options, logger) {
    if (!options.fromPublic) {
        logger.error("--from-public <number> is required for import");
        logger.info("Usage: shirokuma-docs items import --from-public 5");
        return 1;
    }
    ...
    const fetchResult = await runGraphQL(ISSUE_DETAIL_QUERY, {
    ...
    if (projectId && privateIssue.id) {
        ...
        if (itemId) {
            ...
            if (options.priority)
                nonStatusFields["Priority"] = options.priority;
            if (options.size)
                nonStatusFields["Size"] = options.size;
