...
async function resolveLogin(loginOrMe) {
    if (loginOrMe !== "@me")
        return loginOrMe;
 ...
 */
export async function cmdItemAssign(issueNumberStr, userInput, options, logger) {
 ...
 * @me を指定した場合は認証ユーザーに解決する。
 */
export async function cmdItemUnassign(issueNumberStr, userInput, options, logger) {
    const repoInfo = resolveTargetRepo(options);
    if (!repoInfo) {
        logger.error("リポジトリを特定できません");
        return 1;
