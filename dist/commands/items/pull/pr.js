 ...
 */
export async function fetchAndCachePr(owner, repo, number, org, baseDir, logger) {
    const graphqlVars = { owner, name: repo, number };
    // 本体を先に取得（PR かどうかを確認してからコメントを取得する）
    const prResult = await runGraphQL(GRAPHQL_QUERY_PR_WITH_FIELDS, graphqlVars);
    ...
    const commentsResult = await runGraphQL(GRAPHQL_QUERY_PR_COMMENTS, graphqlVars);
    // コメントをキャッシュ書き込み
    if (commentsResult.success && commentsResult.data?.data?.repository?.pullRequest?.comments) {
        ...
        for (const c of nodes) {
            if (c.databaseId) {
                ...
                }, c.body ?? "", "pull_request", org, repo, baseDir);
            }
        }
