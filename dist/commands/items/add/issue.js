...
import { GRAPHQL_MUTATION_CREATE_ISSUE, GRAPHQL_MUTATION_CREATE_LABEL, getRepoId, } from "../../../utils/graphql-queries.js";
import { addItemToProject } from "../../../utils/project-fields.js";
import { getProjectId } from "../../../utils/project-utils.js";
import { getDefaultStatus, loadGhConfig } from "../../../utils/gh-config.js";
import { setFieldsWithStatusRouting } from "../../../utils/issue-detail.js";
import { getLabels, normalizeLabels, resolveIssueTypeId } from "../helpers.js";
import { validateInitialStatus } from "../../../utils/status-workflow.js";
import { writeCache, upsertOpenIssuesEntry, getCachePath, } from "../../../utils/github-cache.js";
 ...
 */
function findMatchingLabelRule(labelName, rules) {
    if (!rules)
        return undefined;
 ...
 */
function matchLabelPattern(pattern, name) {
    if (pattern === "*")
        return true;
    if (pattern.endsWith("*")) {
 ...
 * 失敗した場合は null を返す。
 */
async function createLabelByRule(repoId, labelName, rule, logger, allLabels) {
 ...
 */
export async function cmdAddIssue(options, logger) {
    if (!options.file) {
    ...
    const labelsList = Array.isArray(meta["labels"]) ?
        meta["labels"].filter((l) => typeof l === "string") : [];
    const assigneesList = Array.isArray(meta["assignees"])
        ? meta["assignees"].filter((a) => typeof a === "string")
        : [];
    if (!title) {
        logger.error("frontmatter に title フィールドが必要です");
        return 1;
    }
    // 初期ステータスの検証（Backlog / Pending のみ許可）
    ...
    if (projectId) {
        ...
        if (projectItemId) {
            ...
            if (priorityValue)
                nonStatusFields["Priority"] = priorityValue;
            if (sizeValue)
                nonStatusFields["Size"] = sizeValue;
