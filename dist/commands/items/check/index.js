...
/** ディレクトリ名の一覧 */
const TYPE_DIRS = ["issues", "pulls", "discussions"];
 ...
 */
export async function cmdCheck(numberStr, options, logger) {
 ...
 * 単一アイテムのチェック
 */
export async function checkSingleItem(owner, repo, number, baseDir, logger) {
    ...
    // 差分を比較
    const diffs = compareSnapshots(localMeta, localBody, remoteSnapshot.title, remoteSnapshot.body, remoteSnapshot.status, remoteSnapshot.priority, remoteSnapshot.size, remoteSnapshot.labels);
    return {
        number,
        type,
 ...
 * キャッシュディレクトリ内の全アイテムをチェック
 */
export async function checkAllItems(owner, repo, baseDir, logger) {
