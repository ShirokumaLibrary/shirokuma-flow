 ...
 */
export async function buildLinkGraph(owner, repo, currentPr, linkedIssues, logger) {
 ...
 * 見つからない場合は null を返す。
 */
export async function resolvePrFromHeadWithDetails(headBranch, owner, repo, logger) {
    try {
        const octokit = getOctokit();
        const { data } = await octokit.rest.pulls.list({
            owner,
