 ...
 */
export async function resolveIntegrationBaseBranch(input) {
    ...
    const integrationBranch = await findIntegrationBranch(parentNumber, parentBody ?? undefined);
    if (!integrationBranch) {
        if (!optionsBase) {
            logger.error(`Integration ブランチが見つかりません。--base で明示的に指定してください（例: --base develop）`);
            return null;
        }
        logger.warn(`Integration ブランチが見つかりませんでした。--base の指定（${optionsBase}）を使用します`);
        return {
    ...
    if (optionsBase) {
        logger.warn(`Integration ブランチ \`${integrationBranch}\` が存在しますが --base の指定を優先します（${optionsBase}）`);
        return {
            baseBranch: optionsBase,
            isIntegrationTarget: false,
        };
...
//# sourceMappingURL=integration-branch-resolver.js.map