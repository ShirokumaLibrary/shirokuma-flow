 ...
 * - 70文字以内
 */
function generatePRTitle(title, issueType) {
 ...
 * PR 本文テンプレートを生成する。
 */
function generatePRBody(issueNumber, issueBody) {
 ...
 */
export async function cmdPrCreateFromIssue(numberStr, options, logger) {
    ...
    // Issue 情報を取得
    const issueResult = await runGraphQL(GRAPHQL_QUERY_ISSUE_FOR_PR, { owner, name: repo, number: issueNumber });
...
`;
            const idResult = await runGraphQL(`query($owner: String!, $name: String!, $number: Int!) {
          repository(owner: $owner, name: $name) {
...
//# sourceMappingURL=create-from-issue.js.map