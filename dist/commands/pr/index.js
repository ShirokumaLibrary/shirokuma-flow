...
import { createActionLogger, setExitCode, resolveBodyFileOption, resolveFromFileOption, } from "../../utils/cli-helpers.js";
// =============================================================================
// Factory Function
// =============================================================================
export function createPrCommand() {
    const pr = new Command("pr")
        .description("Pull Request 管理 (create, list, show, comments, merge, close, reply, resolve, edit)");
    // 共通親オプション
    pr
    ...
    pr
        ...
        if (!(await resolveFromFileOption(options, "pr-create")))
            return;
        if (!resolveBodyFileOption(options))
            return;
        // Issue 番号が指定された場合はターゲットブランチ・タイトルを自動判定
        if (number) {
