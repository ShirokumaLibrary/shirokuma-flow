...
// cmdPrEdit (#2119)
// =============================================================================
export async function cmdPrEdit(prNumberStr, options, logger) {
    ...
    if (!hasBase && !hasTitle && !hasBody) {
        logger.error("更新するフィールドを1つ以上指定してください (--base / --title / --body-file)");
        return 1;
    }
    ...
    const updateParams = { owner, repo, pull_number: prNumber };
    if (hasBase)
        updateParams.base = options.base;
    if (hasTitle)
        updateParams.title = options.title;
    if (hasBody)
        updateParams.body = options.bodyFile;
    let updatedPr;
    try {
