...
import { GRAPHQL_QUERY_DISCUSSION, GRAPHQL_QUERY_DISCUSSION_BY_ID, } from "./helpers.js";
 ...
 */
export async function cmdGet(idOrNumber, options, logger) {
    ...
    if (/^\d+$/.test(idOrNumber)) {
        ...
        const [result, commentsResult] = await Promise.all([
            runGraphQL(GRAPHQL_QUERY_DISCUSSION, graphqlVars),
        ...
        if (options.withComments && discussion?.number) {
            const commentsResult = await runGraphQL(GRAPHQL_QUERY_DISCUSSION_COMMENTS, { owner, name: repo, number: discussion.number });
            if (commentsResult.success) {
                commentsData = commentsResult.data?.data?.repository?.discussion?.comments ?? null;
            }
        }
    ...
    if (options.writeCache && discussion.number) {
        ...
        writeCache(discussionNumber, {
            ...
            title: discussion.title,
            updated_at: discussion.updatedAt,
        }, typeof discussion.body === "string" ? discussion.body : "", owner, repo);
        // コメントも個別にキャッシュ書き込み
        if (commentsData) {
            const nodes = commentsData.nodes ?? [];
            for (const c of nodes) {
                if (c.databaseId) {
                    writeCommentCache(discussionNumber, c.databaseId, { number: discussionNumber, database_id: c.databaseId }, typeof c.body === "string" ? c.body : "", "discussion", owner, repo);
                    // 返信もキャッシュ書き込み
                    const replies = c.replies?.nodes ?? [];
                    for (const r of replies) {
                        if (r.databaseId) {
                            writeCommentCache(discussionNumber, r.databaseId, {
                                number: discussionNumber,
                                database_id: r.databaseId,
                                reply_to: c.databaseId,
                            }, typeof r.body === "string" ? r.body : "", "discussion", owner, repo);
                        }
                    }
                }
            }
